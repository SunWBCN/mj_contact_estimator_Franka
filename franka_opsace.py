...
def main() -> None:
    ...
    pino_model = pino.buildModelFromUrdf("franka_emika_panda/panda.urdf")
    pino_data = pino_model.createData()
    print("Pinocchio nq, nv:", pino_model.nq, pino_model.nv)
    print("Pinocchio joints:", pino_model.names)
    ...
    mesh_sampler = MeshSampler(model, data, False, robot_name="franka_emika_panda")
    ...
    with mujoco.viewer.launch_passive(
        ...
        while viewer.is_running():
            ...
            visualize_mat_arrows(scene, geom_pos_world, rot_mat_geom_world, contact_poss_geom, rots_mat_contact_geom,
            ...
            equi_ext_f_poss, equi_ext_wrenches, jacobian_bodies, contact_poss_world, ext_wrenches, jacobian_contacts = \
                mesh_sampler.compute_equivalent_wrenches(contact_poss_geom, rots_mat_contact_geom, normal_vecs_geom,
                                                         sample_body_name,
                                                         geom_id, ext_f_norm)
            ...
            visualize_normal_arrow(scene, equi_ext_f_poss, equi_ext_fs, rgba=np.array([0.0, 1.0, 0.0, 1.0]),
                                   arrow_length=arrow_length)
